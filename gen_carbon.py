...
def portrait_candor(member):
    """Candor — the white coherence core given a face: a serene figure of pure
    white light, haloed by sixteen spectrum emitters (red→violet) whose rays
    converge inward and blend to white at the brow. Sixteen colors, one white."""
    import math
    g=[VOID]*(LW*LH); drawn=[False]*(LW*LH)
    def put(x,y,c):
        x=int(round(x)); y=int(round(y))
    ...
    def hsv(h,s,v):
        ...
        r,gg,b=[(v,t,p),(q,v,p),(p,v,t),(p,q,v),(t,p,v),(v,p,q)][i]
        return (int(r*255),int(gg*255),int(b*255))
    WHITE=(244,240,252); WARM=(255,250,235); MASKSH=(150,150,168)
    cx,cy=32,34
    # the spectrum halo: 16 colored emitters around the head, rays converging to white
    for i in range(16):
        a=-math.pi/2 + i*(2*math.pi/16)
        col=hsv(i/16.0,0.8,1.0)
        ex=cx+26*math.cos(a); ey=22+19*math.sin(a)
        # ray inward, blending to white
        for t in range(0,7):
            tt=t/6.0; xx=ex+(cx-ex)*tt*0.55; yy=ey+(cy-7-ey)*tt*0.55
            soft(xx,yy, mix(col,WHITE,tt), 0.5*(0.4+0.6*tt))
        put(round(ex),round(ey),col); soft(round(ex),round(ey)-1, mix(col,WHITE,0.4),0.7)
    # robe — soft pale (the white spills onto the shoulders)
    robe=(120,120,140); robe_sh=shade(robe,0.3)
    rect_=lambda x0,y0,x1,y1,c:[put(x,y,c) for y in range(y0,y1+1) for x in range(x0,x1+1)]
    rect_(8,58,55,79, robe)
    for x in range(8,56):
        if x<13 or x>50: rect_(x,58,x,60,VOID)
    rect_(20,57,44,60, robe_sh)
    for k in range(7): put(32-k,57+k, robe_sh); put(32+k,57+k, robe_sh)
    # neck
    rect_(28,47,36,57, shade(WHITE,0.25))
    # the white face/core — a glowing oval, brightest at center
    ell(cx,cy,13,17, WHITE)
    for y in range(int(cy-17),int(cy+18)):
        for x in range(int(cx-13),int(cx+14)):
            i=y*LW+x
            if 0<=i<LW*LH and drawn[i] and g[i]==WHITE:
                d=math.hypot((x-cx)/13.0,(y-cy)/17.0)
                g[i]=mix((255,255,255), MASKSH, min(1.0,d*0.9))
    # a bright bloom at the very center (the coherence core)
    ell(cx,cy-2,5,5,(255,255,255))
    # closed serene eyes (faint)
    for x in range(cx-7,cx-2): put(x,cy-1,MASKSH)
    for x in range(cx+3,cx+8): put(x,cy-1,MASKSH)
    # a thin spectrum circlet across the brow (tiny rainbow), blending to white
    for k in range(-10,11):
        col=hsv((k+10)/20.0,0.7,1.0)
        soft(cx+k,cy-14, mix(col,WHITE,0.3),0.8)
    # serene mouth
    for x in range(cx-4,cx+5): put(x,cy+10, MASKSH)
    return finish(g, drawn)


PORTRAITS = {"gravity": portrait_aether, "lattice": portrait_leech, "survival": portrait_survival, "chromatic": portrait_candor}
