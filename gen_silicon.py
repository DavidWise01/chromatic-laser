...
def chromatic_sigil(member):
    """For Candor (the white coherence core): the 16-colour spectrum as a compass
    of emitters around the rim — red through violet, four to a quadrant — each
    firing a ray of its own colour inward to a single pure-white core where they
    all combine. Sixteen colours, one white."""
    VOID=(10,10,18); WHITE=(244,238,252)
    def hsv(h,s,v):
    ...
    for y in range(SIZE):
        for x in range(SIZE):
            d=math.hypot(x-cx,y-cy)/(SIZE*0.5)
            glow=max(0.0,1.0-d*1.2)
            c=mix(VOID, WHITE, 0.10*glow**3)
            c=mix(c, VOID, min(0.5,(d-0.72)*1.7) if d>0.72 else 0.0)
            px[y*SIZE+x]=c
    def plot(x,y,c,a=1.0):
        xi,yi=int(round(x)),int(round(y))
        if 0<=xi<SIZE and 0<=yi<SIZE:
    ...
    def disk(x,y,r,c,a=1.0):
        for yy in range(int(y-r),int(y+r)+1):
            for xx in range(int(x-r),int(x+r)+1):
                if (xx-x)**2+(yy-y)**2<=r*r: plot(xx,yy,c,a)
    def ray(x0,y0,x1,y1,c,a0):
        n=int(max(abs(x1-x0),abs(y1-y0)))+1
        for k in range(n+1):
            t=k/n; a=a0*(0.15+0.85*t)             # brighten toward the white core
            col=mix(c, WHITE, t*t)                 # colour blends to white as it nears center
            disk(x0+(x1-x0)*t, y0+(y1-y0)*t, 1.4, col, a)
    R=SIZE*0.40
    # quadrant guides (4 quadrants, 4 emitters each)
    for q in range(4):
        a=q*math.pi/2 + math.pi/4
        plot(cx+R*1.03*math.cos(a), cy+R*1.03*math.sin(a), (90,90,110), 0.0)
    # 16 colour emitters around the rim, rays inward to the core
    for i in range(16):
        ang=-math.pi/2 + i*(2*math.pi/16)
        col=hsv(i/16.0, 0.85, 1.0)
        ex=cx+R*math.cos(ang); ey=cy+R*math.sin(ang)
        ray(ex,ey, cx,cy, col, 0.5)
        disk(ex,ey, 5.5, col, 0.9); disk(ex,ey, 9, col, 0.16)
    # the pure white coherence core (C17)
    disk(cx,cy, 30, WHITE, 0.10); disk(cx,cy, 18, WHITE, 0.28)
    disk(cx,cy, 10, WHITE, 0.85); disk(cx,cy, 5, (255,255,255), 0.98)
    return px


SIGILS = {"gravity": well_sigil, "lattice": lattice_sigil, "survival": survival_sigil, "chromatic": chromatic_sigil}
for m in R["members"]:
